...
\begin{definition}[前缀不冲突编码]
  \label{def:prefix-free}
  编码 $E:A\to\Sigma^+$ 称为\emph{前缀不冲突的}（prefix-free），如果对
  任意不同的 $x,y\in A$，码字 $E(x)$ 都不是 $E(y)$ 的前缀。
\end{definition}
...
\begin{proof}
  ...
  $E(a_1)$ 与 $E(b_1)$ 中较短的码字一定是较长码字的前缀。由前缀不冲突性可知
  \[
    E(a_1)=E(b_1)
    \implies a_1=b_1.
  \]
...
\begin{theorem}[构造前缀不冲突编码]
  \label{thm:prefix-free-construction}
  若存在编码 $E:A\to\Sigma^*$，则存在前缀不冲突编码
...
\begin{proof}[解答]
  ...
  \[
    ...
    =n+O(\log(n+1)).
  \]
  $L(n)$ 自定界；当 $n$ 相同时，$E'(a)$ 等长。因此该构造前缀不冲突。
\end{proof}
...
\begin{proof}
  先用\cref{thm:prefix-free-construction}把 $E$ 改造成前缀不冲突编码，再用
