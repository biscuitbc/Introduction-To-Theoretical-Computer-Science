...
\]
每个 $T_a$ 使用 $O(n)$ 个门，而这样的项至多有 $2^n$ 个，所以一个输出
比特需要 $O(n2^n)$ 个门。分别构造 $m$ 个输出，便得到
\[
  \card{C}=O(mn2^n).
\]
\cref{thm:all-boolean-functions-have-circuits}中的
$O(m2^n/n)$ 上界去掉了这个直接构造中的冗余。

% 定理 2.2 的精细上界证明留待下一讲补充。
\noindent\textit{精细上界的证明.}
...
\subsection{NAND-Circ 程序中的语法糖}

为了让程序更容易书写，可以暂时加入以下三种\emph{语法糖}：

\begin{enumerate}[label=\arabic*.]
  \item \textbf{定长循环：}把循环体复制固定次数；
  \item \textbf{用户定义过程：}在调用处展开过程体；
  \item \textbf{条件语句：}同时计算两个分支，再用选择器输出其中一个。
\end{enumerate}
...
\begin{example}[ADD]
  ...
  \]
  令 $\operatorname{MAJ}(a,b,c)$ 表示三个比特的多数值，即
  \[
