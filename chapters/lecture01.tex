...
的一种实现。从函数到程序，就是把抽象的东西具体化。本讲先用二进制串
形式化“描述”，再比较函数与程序在数量上的差异。
...
\begin{definition}[二进制串]
  ...
  串联记为 $xy$。对 $x,y\in\Sigma^*$，定义
  \[
    x\preceq y
    \iff \exists z\in\Sigma^*,\ y=xz,
    \qquad
    x\prec y
    \iff x\preceq y\ \text{且}\ x\neq y.
  \]
  分别称 $x$ 是 $y$ 的\emph{前缀}和\emph{真前缀}。
\end{definition}
...
\begin{definition}[无前缀编码]
  ...
  编码 $E:A\to\Sigma^+$ 称为\emph{无前缀的}（prefix-free），如果
  \[
...
\begin{proof}
  ...
  $E_{\mathrm{pf}}(a)\preceq E_{\mathrm{pf}}(b)$，则短串的终止块必须与
  长串的某一块对齐。正文中没有 $01$，故它只能与长串的终止块对齐。于是
  \[
...
\begin{definition}[至多可数]
  ...
  集合 $A$ \emph{至多可数}，是指
  \[
    \exists\text{ 单射 }f:A\hookrightarrow\Nat.
  \]
  等价地，$A$ 有限或 $A\cong\Nat$。当 $A\neq\varnothing$ 时，还等价于
  \[
    \exists\text{ 满射 }g:\Nat\twoheadrightarrow A.
  \]
\end{definition}
...
\begin{proof}
  ...
  若 $E:A\hookrightarrow\Sigma^*$，则
  \[
    \nu\circ E:A\hookrightarrow\Nat.
  \]
  反之，若 $f:A\hookrightarrow\Nat$，令 $c(n)=1^n0$，则
  \[
    c\circ f:A\hookrightarrow\Sigma^*.
  \]
\end{proof}
