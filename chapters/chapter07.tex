...
\begin{definition}[图灵机的性质；语义与非平凡]
  \label{def:semantic-nontrivial-property}
  图灵机的一个\emph{性质} $P$ 为每台图灵机 $M$ 指定一个布尔值
  $P(M)\in\bits$。其中 $P(M)=1$ 表示 $M$ 满足性质 $P$，$P(M)=0$ 表示
  $M$ 不满足性质 $P$。

  性质 $P$ 称为\emph{语义的}，如果任意两台计算同一个偏函数
  ...
  \emph{非平凡的}（non-trivial）。换言之，非平凡也就是 $P$ 不是常函数。
\end{definition}
...
\begin{proof}
  令 $M_\infty$ 为在每个输入上都永远循环的机器。

  \begin{pseudocode}{Diverging machine $M_\infty$ (input $y$)}
    \item Loop forever.
  \end{pseudocode}
  ...
  \begin{pseudocode}{Reduction machine $N_{m,x}$ (input $y$)}
    \item If $m$ is not a valid machine encoding, loop forever.
    \item Simulate $M_m(x)$ until it halts.
    \item Simulate $M_{\mathrm{yes}}(y)$ and output its result.
  \end{pseudocode}
...
\begin{proof}
  \textbf{构造思路.}
  若直接要求一台机器把自己的编码写在程序里，就会产生循环依赖。为此把目标
  机器拆成先后运行的两部分 $R=A_b\to B$。其中 $b=\langle B\rangle$，而
  $A_b$ 只需硬编码 $b$，不必预先知道 $\langle R\rangle$。运行时，$A_b$
  先把 $b$ 连同原输入 $x$ 写成 $\langle b,x\rangle$；$B$ 再由 $b$ 计算
  $q(b)=\langle A_b\rangle$，并把两部分的编码组合起来：
  ...
  这正是笔记中“$A$ 写出 $\langle B\rangle$，$B$ 再生成
  $\langle A\rangle$ 并与 $\langle B\rangle$ 组合”的想法。
  ...
  最后一个等号并非额外假设：$R$ 按构造正是编码为 $q(b)$ 的 $A_b$ 与编码为
  $b$ 的 $B$ 的串联，所以代码生成函数 $c$ 保证
  $r=c(q(b),b)$ 恰好就是 $\langle R\rangle$。因此 $R$ 输出的确实是自己的
  编码。
\end{proof}
...
\begin{proof}
  \textbf{构造思路.}
  递归定理可以看成把自打印机器产生的自身编码直接交给 $T$。按照笔记中的
  分解，令
  ...
  $A$ 只需硬编码 $\langle B\rangle$ 与 $\langle T\rangle$，并保留原输入
  $x$；$B$ 再由这些编码生成 $\langle A\rangle$，并重新排列纸带，使其内容
  变为
  ...
  最后的 $T$ 因而收到的恰是 $(\langle R\rangle,x)$，输出
  $T(\langle R\rangle,x)$。下面的形式化构造把笔记中的 $B\to T$ 合并为
  一台机器 $B_T$。
