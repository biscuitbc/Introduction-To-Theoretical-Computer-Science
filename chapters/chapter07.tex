...
\begin{proof}
  ...
  若 $\operatorname{HALT}_0$ 可计算，等式右侧便给出了停机问题的算法，与
...
\begin{proof}
  假设 $\operatorname{ZERO}$ 可计算。给定 $m,x$，构造机器 $N_{m,x}$：在任意
...
\begin{pseudocode}{参数写入机器 $A_w$（输入 $x$）}
  \item 保留输入 $x$，并把固定字符串 $w$ 写到工作带上。
  \item 输出 $\langle w,x\rangle$ 并停机。
\end{pseudocode}
...
\begin{proof}
  ...
  \begin{pseudocode}{机器 $B$（输入 $\langle w,x\rangle$）}
    \item 计算 $a\gets q(w)$，即得到 $A_w$ 的编码。
    \item 计算 $r_w\gets c(a,w)$。
    \item 输出 $r_w$ 并停机。
  \end{pseudocode}
  ...
  \begin{pseudocode}{自打印机器 $R$（输入 $x$）}
    \item 运行 $A_b(x)$，得到 $\langle b,x\rangle$。
    \item 在该结果上运行 $B$，并原样输出 $B$ 的结果。
  \end{pseudocode}
...
\begin{proof}
  ...
  \begin{pseudocode}{递归定理中的机器 $B_T$（输入 $\langle w,x\rangle$）}
    \item 计算 $a\gets q(w)$。
    \item 计算 $r_w\gets c(a,w)$。
    \item 模拟 $T(r_w,x)$，并原样输出其结果。
  \end{pseudocode}

  \begin{pseudocode}{不动点机器 $R$（输入 $x$）}
    \item 运行 $A_b(x)$，得到 $\langle b,x\rangle$，其中
      $b=\langle B_T\rangle$。
    \item 在该结果上运行 $B_T$，并原样输出其结果。
  \end{pseudocode}
...
\begin{example}[用递归定理重证停机问题]
  ...
  \begin{pseudocode}{反证机器 $R$（输入 $x$）}
    \item 取得自己的编码 $m\gets\langle R\rangle$。
    \item 计算 $h\gets H(m,x)$。
    \item 若 $h=1$，则永远循环；若 $h=0$，则输出 $0$ 并停机。
  \end{pseudocode}
...
\begin{proof}
  ...
  \begin{pseudocode}{识别最小机器的反证机器 $R$（输入 $x$）}
    \item 取得自己的编码 $m\gets\langle R\rangle$。
    \item 按长度枚举 $s\in\bitsstar$，直到
      $\operatorname{MIN}(s)=1$ 且 $\card{s}>\card{m}$。
    \item 在 $x$ 上模拟 $M_s$，并原样输出其结果。
  \end{pseudocode}
...
\begin{proof}
  ...
  \begin{pseudocode}{不动点机器 $F$（输入 $x$）}
    \item 取得自己的编码 $e\gets\langle F\rangle$。
    \item 计算 $m\gets f(e)$。
    \item 在 $x$ 上模拟 $M_m$，并原样输出其结果。
  \end{pseudocode}
\end{proof}
...
\begin{proof}
  ...
  \begin{pseudocode}{由完备证明系统构造的停机判定机 $D$（输入 $m$）}
    \item 若 $m$ 不是合法图灵机编码，则输出 $0$；否则解码得到 $M$。
    \item 交错运行两条分支：第一条逐步模拟 $M(0)$；第二条枚举
      $p\in\bitsstar$ 并计算 $V(n_M,p)$。
    \item 若第一条分支发现 $M(0)$ 停机，则输出 $1$；若第二条分支发现
      $V(n_M,p)=1$，则输出 $0$。
  \end{pseudocode}
...
\begin{proof}
  ...
  \begin{pseudocode}{利用 $\operatorname{EVAL}$ 判定停机的机器 $D_0$
      （输入 $m$）}
    \item 若 $m$ 不是合法图灵机编码，则输出 $0$。
    \item 由 \cref{lem:arithmetization-of-computation} 计算
      $\varphi_{M_m}$。
    \item 输出 $\operatorname{EVAL}(\varphi_{M_m})$。
  \end{pseudocode}
...
\begin{proof}
  ...
  \begin{pseudocode}{由完备证明系统构造的 QIS 判定机 $D_{\mathrm{QIS}}$
      （输入 $\varphi$）}
    \item 若 $\varphi$ 不是合法 QIS，则输出 $0$。
    \item 交错枚举 $\varphi$ 与 $\lnot\varphi$ 的候选证明 $p$，并计算相应的
      $V(\varphi,p)$ 与 $V(\lnot\varphi,p)$。
    \item 若找到 $\varphi$ 的证明，则输出 $1$；若找到 $\lnot\varphi$ 的证明，
      则输出 $0$。
  \end{pseudocode}
\end{proof}
