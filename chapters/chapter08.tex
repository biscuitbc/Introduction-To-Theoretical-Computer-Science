...
\begin{proof}[证明思路]
  NAND-RAM 用数组保存纸带，并用一个整数记录读写头位置；一次图灵机转移可由
  常数行程序模拟。反过来，图灵机把 NAND-RAM 已使用的寄存器和内存单元编码
  在纸带上。若程序运行 $t$ 行，则至多需要处理 $O(t)$ 个已访问位置；逐条扫描、
  更新并执行有限位运算，可把总模拟时间控制在 $O(t^4)$。当 $T(n)\geq n$ 且
  $n$ 足够大时，两个方向的固定开销分别由常数 $10$ 与 $C$ 统一控制。
\end{proof}
