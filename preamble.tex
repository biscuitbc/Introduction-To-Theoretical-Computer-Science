\usepackage[a4paper,margin=2.6cm,headheight=14pt]{geometry}
\usepackage{amsmath,amssymb,mathtools}
\usepackage{amsthm}
\usepackage{enumitem}
\usepackage{xcolor}
\usepackage{tikz}
\usetikzlibrary{automata,positioning}
\usepackage{microtype}
\usepackage[unicode,colorlinks=true,linkcolor=blue!55!black,urlcolor=blue!55!black]{hyperref}
\usepackage[nameinlink,noabbrev]{cleveref}

\ctexset{
  chapter = {
    name = {第,章},
    number = \arabic{chapter},
    aftername = \quad
  }
}

\setcounter{tocdepth}{2}
\setcounter{secnumdepth}{2}

% Lecture 与 chapter 相互独立；一个 chapter 可以包含多次 lecture。
\newcounter{lecture}
\newcommand{\lecture}{%
  \refstepcounter{lecture}%
}

% 各类环境使用独立计数器，并分别按章重新编号。
\theoremstyle{plain}
\newtheorem{theorem}{定理}[chapter]
\newtheorem{lemma}{引理}[chapter]
\newtheorem{corollary}{推论}[chapter]
\newtheorem{proposition}{命题}[chapter]

\theoremstyle{definition}
\newtheorem{definition}{定义}[chapter]
\newtheorem{example}{例}[chapter]

\theoremstyle{remark}
\newtheorem*{remark}{注}
\newtheorem*{question}{思考题}

\crefname{theorem}{定理}{定理}
\crefname{lemma}{引理}{引理}
\crefname{corollary}{推论}{推论}
\crefname{proposition}{命题}{命题}
\crefname{definition}{定义}{定义}
\crefname{example}{例}{例}
\crefname{equation}{式}{式}
\crefname{section}{节}{节}
\crefname{lecture}{Lecture}{Lectures}
\crefformat{chapter}{第#2#1#3章}

\newcommand{\bits}{\{0,1\}}
\newcommand{\bitsplus}{\bits^{+}}
\newcommand{\bitsstar}{\bits^{*}}
\newcommand{\eps}{\varepsilon}
\newcommand{\card}[1]{\lvert #1\rvert}
\newcommand{\Nat}{\mathbb{N}}

% 讲义中的机器构造统一写成紧凑的伪代码块。
\newenvironment{pseudocode}[1]
  {\begin{quote}\begin{minipage}{\linewidth}\small
   \noindent\textbf{Pseudocode: #1}\par\smallskip
   \begin{enumerate}[label=\arabic*.,leftmargin=2.2em,itemsep=0.25em,topsep=0pt]}
  {\end{enumerate}\end{minipage}\end{quote}}

\renewcommand{\proofname}{证明}
